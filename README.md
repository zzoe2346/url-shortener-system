...
- 도전 과제
  - 비동기 전환으로 처리량을 높이는 데는 성공했지만, 예상보다 더 높은 부하가 발생하자 스레드 풀의 작업 큐(Queue)가 가득 차 `TaskRejectedException`이 발생하며 시스템이 불안정해지는 새로운 문제 발생.
 
- 해결 방안
  - 시스템이 붕괴하지 않고 스스로를 보호할 수 있도록, 스레드 풀의 거부 정책(Rejection Policy)을 기본 `AbortPolicy`에서 `CallerRunsPolicy`로 변경.
